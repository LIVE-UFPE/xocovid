...
png(file = img, width=650, height = 500, units = 'px')
plot(forecast1, lwd =2, type='l',ylab="Casos confirmados", xlab="Tempo", main=paste0("Predição dos casos confirmados da covid-19 em Recife-PE de ",dateFormat(data$dt_notificacao[length(tsdata) - 5])," a ",dateFormat(data$dt_notificacao[length(tsdata)])))
grid(lwd = 2, col = 'blue') 
dev.off()
...
main=paste0("Projeção dos casos confirmados da covid-19 em Recife-PE de ",begin," a ",end),
)
grid(lwd = 2, col = 'blue') 
dev.off()
