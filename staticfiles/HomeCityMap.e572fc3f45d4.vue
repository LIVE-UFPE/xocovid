...
module.exports ={
    ...
    mounted() {
        ...
        this.request = $.ajax({
            ...
            url: "graphs/get_data",
            // TODO rodar a busca no views.py
            data: {"informacao": 'statesData', "keyBusca": '', "estado": this.estadoComp, "cidade": '', "bairro": ''},
            success: function (response) {
                resposta = response
            }
        })
        //this.geojson = L.geoJson(eval(this.estadoComp), {style: this.style, onEachFeature: onEachFeature.bind(this)})
        this.geojson = L.geoJson(resposta, {style: this.style, onEachFeature: onEachFeature.bind(this)})
    ...
    computed: {
        ...
        estadoComp() {
            ...
            return this.estado.slice(0,this.estado.length-5).split(' ').join(' ')
        }
    },
    watch: {
        datewatch() {
